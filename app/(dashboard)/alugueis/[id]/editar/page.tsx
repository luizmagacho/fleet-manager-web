...
export default function EditRentalPage() {
  ...
  useEffect(() => {
    if (rental) {
      setForm({
        vehicleId: rental.vehicleId?._id || rental.vehicleId || '',
        driverId: rental.driverId?._id || rental.driverId || '',
        startDate: formatISOToPtBr(rental.startDate),
        expectedEndDate: formatISOToPtBr(rental.expectedEndDate || rental.endDate),
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof form) => api.put(`/rentals/${id}`, data),
    onSuccess: () => {
      toast.success('Contrato de aluguel atualizado com sucesso!');
  ...
  const isLoading = rentalLoading || vehiclesLoading || driversLoading;
  ...
  const inputClass = "w-full rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm focus:border-red-500 focus:outline-none dark:border-slate-700 dark:bg-slate-900 dark:text-white";
  const labelClass = "mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300";

  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/alugueis/${id}`} className="rounded-lg p-2 hover:bg-slate-100 dark:hover:bg-slate-800">
          <ArrowLeft className="h-5 w-5 text-slate-500" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Editar Aluguel</h1>
          <p className="text-sm text-slate-500">Edite as informações e datas do contrato de locação</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <Link href={`/alugueis/${id}`} className="rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300">
            Cancelar
          </Link>
          <button
            type="submit"
